function [Output]=CLEF(Input,Mode)
threshold1=exp(-1/2);
threshold2=1/2;
chunksize=200;
...
rho0=0.5;
if strcmp(Mode,'L')==1
    [Output.Ye,Output.Syst.ModelNumber,Output.Syst.prototypes,Output.Syst.center,Output.Syst.Support,Output.Syst.local_delta,Output.Syst.Global_mean,Output.Syst.Global_X,Output.Syst.A,Output.Syst.L]=EFStrain(Input.x,Input.y,threshold1,threshold2,chunksize,forgettingfactor,rho0);
end
if strcmp(Mode,'T')==1
    [Output.Ye]=EFStest(Input.x,Input.Syst.ModelNumber,Input.Syst.prototypes,Input.Syst.center,Input.Syst.local_delta,Input.Syst.Global_mean,Input.Syst.Global_X,Input.Syst.A,Input.Syst.L,threshold1,threshold2,chunksize);
end
...
[L,W]=size(datack0);
X=[ones(L,1),datack0];
X0=reshape(X',W+1,1,L);
X1=pagemtimes(X0,pagetranspose(X0));
for jj=1:1:ModelNumber
    C(:,:,jj)=C(:,:,jj)+sum(X1.*reshape(centerlambda(:,jj),1,1,L),3);
    A(:,:,jj)=A(:,:,jj)+(Yck0-X*A(:,:,jj)')'*(centerlambda(:,jj).*X)*pinv(C(:,:,jj));
end
end